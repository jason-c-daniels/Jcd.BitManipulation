...
public class BitIndexerUInt16Tests
{
   ...
   public void Implicit_Operator_To_BitIndexerUInt16_From_UInt16_Sets_All_Bits_Correctly(ushort data)
   {
      BitIndexer indexer = data;
      Assert.Equal(data, (ushort) indexer);
   }
   ...
   public void Indexer_Returns_Correct_Bit_Value(ushort data)
   {
      BitIndexer indexer = data;
      ushort mask = 0;
   ...
   public void Indexer_Sets_Correct_Bit_Value(int index)
   {
      ...
      var expected = ((ushort) 0).SetBit(index);
      indexer[index] = true;
   ...
   public void Slice_Returns_Correct_Subset_Of_Bools(ushort data, int start, int end)
   {
      BitIndexer indexer = data;
