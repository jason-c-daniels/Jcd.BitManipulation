...
public class ByteArrayExtensionTests
{
    ...
    [Theory]
    [InlineData(0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, Endian.Little, 0x0807060504030201)]
    [InlineData(0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, Endian.Big, 0x0807060504030201)]
    public void ToUInt64_On_Array(byte d0, byte d1, byte d2, byte d3, byte d4, byte d5, byte d6, byte d7, Endian endian,
                                  ulong expected)
    {
        Assert.Equal(expected, new[] { d0, d1, d2, d3, d4, d5, d6, d7 }.ToUInt64(endian));
    }

    [Theory]
    [InlineData(0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, Endian.Little, 0x0807060504030201)]
    [InlineData(0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, Endian.Big, 0x0807060504030201)]
    public void ToInt64_On_Array(byte d0, byte d1, byte d2, byte d3, byte d4, byte d5, byte d6, byte d7, Endian endian,
                                 long expected)
    {
        Assert.Equal(expected, new[] { d0, d1, d2, d3, d4, d5, d6, d7 }.ToInt64(endian));
    }
    ...
    #region To[Integer Data Type] Tests ReadOnlySpan of bytes
    ...
    [Theory]
    [InlineData(0x01, 0x02, Endian.Little, 0x0201)]
    [InlineData(0x02, 0x01, Endian.Big, 0x0201)]
    public void ToUInt16_On_ReadOnlySpan(byte d0, byte d1, Endian endian, ushort expected)
    {
        Assert.Equal(expected, new ReadOnlySpan<byte>(new[] { d0, d1 }).ToUInt16(endian));
    }

    [Theory]
    [InlineData(0x01, 0x02, Endian.Little, 0x0201)]
    [InlineData(0x02, 0x01, Endian.Big, 0x0201)]
    public void ToInt16_On_ReadOnlySpan(byte d0, byte d1, Endian endian, short expected)
    {
        Assert.Equal(expected, new ReadOnlySpan<byte>(new[] { d0, d1 }).ToInt16(endian));
    }

    [Theory]
    [InlineData(0x01, 0x02, 0x03, 0x04, Endian.Little, 0x04030201)]
    [InlineData(0x04, 0x03, 0x02, 0x01, Endian.Big, 0x04030201)]
    public void ToUInt32_On_ReadOnlySpan(byte d0, byte d1, byte d2, byte d3, Endian endian, uint expected)
    {
        Assert.Equal(expected, new ReadOnlySpan<byte>(new[] { d0, d1, d2, d3 }).ToUInt32(endian));
    }

    [Theory]
    [InlineData(0x01, 0x02, 0x03, 0x04, Endian.Little, 0x04030201)]
    [InlineData(0x04, 0x03, 0x02, 0x01, Endian.Big, 0x04030201)]
    public void ToInt32_On_ReadOnlySpan(byte d0, byte d1, byte d2, byte d3, Endian endian, int expected)
    {
        Assert.Equal(expected, new ReadOnlySpan<byte>(new[] { d0, d1, d2, d3 }).ToInt32(endian));
    }
    ...
    {
        Assert.Equal(expected, new ReadOnlySpan<byte>(new[] { d0, d1, d2, d3, d4, d5, d6, d7 }).ToUInt64(endian));
    }
    ...
    {
        Assert.Equal(expected, new ReadOnlySpan<byte>(new[] { d0, d1, d2, d3, d4, d5, d6, d7 }).ToInt64(endian));
    }
