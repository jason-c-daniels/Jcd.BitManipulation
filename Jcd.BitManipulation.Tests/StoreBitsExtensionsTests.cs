...
public class StoreBitsExtensionsTests
{
    ...
    [InlineData(0x0000, 0xFFFF, 0, 16, 0xFFFF)]
    [InlineData(0xF000, 0x03, 0, 8, 0xF003)]
    public void StoreBits_On_Int64_Stores_The_Correct_Bits(ulong initial, ulong val, int offset, int size,
                                                           ulong expected)
    {
    ...
    [InlineData(0x0000, 0xFFFF, 0, 16, 0xFFFF)]
    [InlineData(0xF000, 0x03, 0, 8, 0xF003)]
    public void StoreBits_On_UInt64_Stores_The_Correct_Bits(ulong initial, ulong val, int offset, int size,
                                                            ulong expected)
    {
        var result = initial.StoreBits(val, offset, size);
        Assert.Equal(expected, result);
